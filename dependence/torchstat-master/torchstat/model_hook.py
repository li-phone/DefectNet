...
class ModelHook(object):
    def __init__(self, model, input):
        ...
        if isinstance(self._input, (list, tuple)):
            x = torch.rand(1, *self._input)
            self._model(x)
        elif isinstance(self._input, dict):
            x = self._input  # add module duration time
            result = self._model(return_loss=False, rescale=True, **x)
            if isinstance(result, int):
                print('predict result is 0')
            else:
                print('predict result is 1')
